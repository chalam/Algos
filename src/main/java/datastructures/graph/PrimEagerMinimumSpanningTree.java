...
/**
 * Improves the lazy by pruning the MinPQ with only eligible crossing edges
 * Start with vertex 0 and greedily grow MST T
 * Add to T find the min weight edge with exactly one endpoint in T
 * Maintain PQ of vertices connected by edge to T
 * at most only one entry per vertex
 * and priorty of vertex v = weight of the shortest edge
 * Repeat until v-1 edges in MST
 * Space = E
 * TIme Complexity = O((|V| + |E|) log |V|) = O(|E| log |V|)
 * Created by Lamuel on 4/22/2016.
 */
public class PrimEagerMinimumSpanningTree {
     ...
     */
    public PrimEagerMinimumSpanningTree(EdgeWeightedGraph G) {
        edgeTo = new Edge[G.V()];
